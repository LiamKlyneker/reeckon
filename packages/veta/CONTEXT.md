...
The `veta` npm package provides a CLI (`veta dev`, `veta build`, `veta add`) and a React viewer SPA for browsing AI skill definitions (SKILL.md files).
...
The viewer shows a "copy install command" (`npx veta add <repo> --skill <slug>`) on each skill detail page. The repo URL is resolved at startup:
1. **Explicit override** — `veta.repo` field in `package.json` (e.g. `"veta": { "repo": "acme/ai-skills" }`)
...
# Test veta add (local source)
cd test/consumer-project && node ../../packages/veta/dist/cli/index.js add ../../test/sample-project --skill code-review --tool claude-code,cursor

# Test veta add (GitHub repo)
cd test/consumer-project && node ../../packages/veta/dist/cli/index.js add vercel-labs/agent-skills
```
